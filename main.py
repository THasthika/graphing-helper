...
def write_file(fname, scales, coords, datapoints, gradient, intersect, fitcoords):
...
def find_scale(rang, spaces):
    ...
    sv = round((vmax - vmin) / spaces, 1)
    sv_small = round(sv / 10, 2)
...
def find_coords(scales):
    ...
    for i in range(scales['spaces']):
        arr.append(round(scales['scale'] * i + scales['min'], 1))
    return arr
...
def find_nearest_coord(xval, yval, xscales, yscales, xcoords, ycoords):
    ...
    if xoffseti > 10:
        ...
        xcoord = round(xcoord + xscales['scale'], 1)
        xoffseti = 0

    if yoffseti > 10:
        ycoord = round(ycoord + yscales['scale'], 1)
        yoffseti = 0
...
def generate_fitline_coords(xdata, ydata, xscales, yscales, xcoords, ycoords):
    ...
    for i in xcoords:
        yval = round(m * i + b, 1)
        coords.append(find_nearest_coord(i, yval, xscales, yscales, xcoords, ycoords))
...
if __name__ == "__main__":
    ...
    write_file(sys.argv[2], scales, coords, datapoints, gradient, intersect, fitcoords)
